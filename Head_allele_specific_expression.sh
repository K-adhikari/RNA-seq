...
STAR --genomeDir --readFilesIn CSrab-18C-Head_S7.fq,CSrab-18C-Head_S8.fq,CSrab-18C-Head_S9.fq --runThreadN 4 > 1pass.out 
...
STAR --genomeDir /2pass/ --readFilesIn CSrab-18C-Head_S7.fq,CSrab-18C-Head_S8.fq,CSrab-18C-Head_S9.fq --runThreadN 4 > Finalpass.out



## Add read group information, sort, mark duplicates and index

java -Xmx8g -jar -Djava.io.tmpdir= /tmp/ picard-tools-1.133/picard.jar AddOrReplaceReadGroups I=/FinalPass/Aligned.out.sam O= rg_added_sorted.bam SO=coordinate RGID=1 RGLB=library RGPL=illumina RGPU=machine RGSM=CSrab18Chead TMP_DIR= /tmp/ > AddOrReplaceReadGroups.out
...
# Variant calling
java -Xmx8g -jar -Djava.io.tmpdir=/tmp/ GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar -T HaplotypeCaller -R mds_ref_Musca_domestica-2.0.2_names.fa -I recal_reads.bam -dontUseSoftClippedBases -stand_call_conf 20.0 -stand_emit_conf 20.0 -o CSrab18Chead_VariantCall.vcf > CSrab18Chead_VariantCall.out

# Variant filtering
java -Xmx8g -jar -Djava.io.tmpdir=/tmp/ GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar -T VariantFiltration -R mds_ref_Musca_domestica-2.0.2_names.fa -V CSrab18Chead_VariantCall.vcf -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o CSrab18Chead_VariantFilter.vcf > CSrab18Chead_VariantFilter.out

# Output records for joint genotyping
java -Xmx8g -jar -Djava.io.tmpdir= /tmp/ GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar -T HaplotypeCaller -R mds_ref_Musca_domestica-2.0.2_names.fa -I recal_reads.bam -o CSrab18Chead_VariantCall.g.vcf -ERC GVCF > CSrab18Chead_VariantCall_gvcf.out 


######################
### We repeated all of the above steps for CSrab at 29°C, IsoCS at 18°C, and IsoCS at 29°C and we used the g.vcf files thus generated for joint genotyping
######################



## Genotype 
java -Xmx8g -jar -Djava.io.tmpdir=/tmp/ GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar -T GenotypeGVCFs -R mds_ref_Musca_domestica-2.0.2_names.fa -V CSrab18Chead_VariantCall.g.vcf -V CSrab29Chead_VariantCall.g.vcf -V IsoCS18Chead_VariantCall.g.vcf -V IsoCS29Chead_VariantCall.g.vcf -stand_call_conf 20.0 -stand_emit_conf 20.0 -o Head_JointGenotype.vcf > Head_JointGenotype.out 

# Filter variants from Joint Genotyping 
java -Xmx8g -jar -Djava.io.tmpdir=/tmp/ GenomeAnalysisTK-3.4-0/GenomeAnalysisTK.jar -T VariantFiltration -R mds_ref_Musca_domestica-2.0.2_names.fa -V Head_JointGenotype.vcf -window 35 -cluster 3 -filterName FS -filter "FS > 30.0" -filterName QD -filter "QD < 2.0" -o Head_JointGenotype_Filtered.vcf > Head_JointGenotype_Filtered.out 

### Writing filtered variants into a new csv file
awk -F '\t' '{if($0 ~ /\#/) print; else if($7 == "PASS") print}' Head_JointGenotype_Filtered.vcf > Head_JointGenotype_Filtered_Pass.vcf
